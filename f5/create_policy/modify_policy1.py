...
def modify_or_load_policy(f5_conn, policy_name, strategy):
    """
    Modify an existing policy or load it if it exists, checking both the Common partition and Drafts.
    """
    try:
        # Try to load the policy from the Common partition
        return f5_conn.tm.ltm.policys.policy.load(name=policy_name, partition='Common')
    except Exception as e:
        logger.error(f"Policy not found in Common partition, attempting to load from Drafts: {e}")
        try:
            # Try to load the policy from the Drafts subpath
            return f5_conn.tm.ltm.policys.policy.load(name=policy_name, partition='Common', subPath='Drafts')
        except Exception as e:
            logger.error(f"Policy not found in Drafts, attempting to create a new one: {e}")
            try:
                # Create a new policy
...
def main():
    ...
    if f5_connection:
        pools_file_path = ('/Users/eliasafabargel/Library/Mobile '
                           'Documents/com~apple~CloudDocs/DevSecOps/f5/pools_server/pool-name-server.txt')  # Update with
        # your file path
        policy_name = 'comaxerp_Comaxdll'
